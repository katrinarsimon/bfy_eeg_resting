...
%%NEED LAB NOTES: 
%This script is the NORMAL MADE (i.e, not miniMADE) pipeline with ICA (ADJUST) commented out
%for BFY Age 4 RESTING 
%weekly preprocessing QC checks. This script SHOULD NOT BE USED for final data
%analysis. Note changes from our usual pipeline include diffferent voltage
%thresholds, NOT overlapping epochs

%again, note this is the RESTING script
...
% Comment out by site 
rawdata_location = '/Volumes/biobehavioraldata/Noble/BFY/Age 4/EEG/Raw Data/NE_EEG/Resting'; % NE
% rawdata_location = '/Volumes/biobehavioraldata/Noble/BFY/Age 4/DATA_AGE 4/EEG/Resting'; %Actual path (MAC) will be 
...
output_location = '/Volumes/biobehavioraldata/Noble/BFY/Age 4/EEG/Processed/resting_processed/all';

% 3. Enter the path of the channel location file
channel_locations = '/Volumes/biobehavioraldata/Noble/BFY/Age 4/EEG/MADE/Toolboxes/MADE-EEG-preprocessing-pipeline-master/channel location files/GSN-HydroCel-129.sfp';
...
task_event_markers = {'stm+'}; % enter all the event/condition markers

% 9. Do you want to epoch/segment your data?
epoch_data = 1; % 0 = NO (do not epoch), 1 = YES (epoch data)
%task_epoch_length = [0 1]; % epoch length in second
...
%EEG_chan = pop_loadset('filename','OIT125_MMN_10222019.set','filepath','X:\Projects\MADE pipeline project\Waveletting\raw data\original_eight');
    %EEG_chan = eeg_checkset(EEG_chan); 

   % chans_labels=cell(1,EEG_chan.nbchan);
   % for i=1:EEG_chan.nbchan
     %   chans_labels{i}= EEG_chan.chanlocs(i).labels;
   % end
   ...
   % if delete_outerlayer==1
     ...
     %   else
         %   EEG_chan = pop_select( EEG_chan,'nochannel', outerlayer_channel_idx);
         %   EEG_chan = eeg_checkset( EEG_chan );
     %   end
   % end
...
%     cd('L:\Noble\BFY\Age 4\EEG\Processed\Scripts');
%     HBCD_merge_filter_faster();
% 
% datafile_names=dir(['L:\Noble\BFY\Age 4\EEG\Processed\filtered_data' filesep '*.set']);
% datafile_names=datafile_names(~ismember({datafile_names.name},{'.', '..', '.DS_Store'}));
% datafile_names={datafile_names.name};
% %[filepath,name,ext] = fileparts(char(datafile_names{1}));
...
%     
    ...
    if numel(FASTbadChans)==0
        ...
        total_channels_interpolated(subject)=numel(FASTbadChans);
    end
...
%Note this is a condensed version of the MADE table, but a slightly longer
%version of the minimade table

cd('/Volumes/biobehavioraldata/Noble/BFY/Age 4/EEG/Processed/resting_processed')


report_table=table(datafile_names', reference_used_for_faster', faster_bad_channels', ...
     total_epochs_before_artifact_rejection', total_epochs_after_artifact_rejection',total_channels_interpolated');
